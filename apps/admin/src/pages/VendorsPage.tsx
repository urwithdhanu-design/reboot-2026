  ...
  return (
    <AdminLayout>
      <PageHeader
        ...
        subtitle="Onboard third-party cover providers (e.g. Vitality for Health), publish UI + services, and email credentials."
        actions={
      ...
      {notice ? (
        <div className="mb-4 rounded-xl bg-lbg-green-light text-lbg-green-dark px-4 py-3 text-sm font-medium">
          {notice}
        </div>
      ) : null}
      ...
      <Card padding={false}>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wide text-lbg-gray-400 border-b border-lbg-gray-100">
                <th scope="col" className="px-5 py-3">Vendor</th>
                <th scope="col" className="px-5 py-3">Categories</th>
                <th scope="col" className="px-5 py-3">Status</th>
                <th scope="col" className="px-5 py-3">UI deploy</th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td className="px-5 py-6 text-lbg-gray-400" colSpan={5}>
                    Loading vendors…
                  </td>
                </tr>
              ) : vendors.length === 0 ? (
                <tr>
                  <td className="px-5 py-6 text-lbg-gray-400" colSpan={5}>
                    No vendors yet. Onboard Vitality or another partner to get started.
                  </td>
                </tr>
              ) : (
                vendors.map((vendor) => (
                  <tr key={vendor.id} className="border-b border-lbg-gray-50">

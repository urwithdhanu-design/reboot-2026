...
function QuoteSummary({ quote, premium }: { quote: QuoteEstimate; premium: number }) {
  return (
...
export function ReviewQuoteModal({
  ...
  async function payWithWallet() {
    ...
    setLoading("wallet");
    try {
      stashQuote(activeQuote);
      const res = await api.payWithWallet(token, activeQuote.quote_id);
      if (!res.paid) {
  ...
  async function startCheckout() {
    ...
    try {
      stashQuote(activeQuote);
      const session = await api.createCheckout(activeQuote.quote_id);
      if (!session.url) {
  ...
  return createPortal(
    <div className="review-quote-backdrop" role="presentation" onClick={onClose}>
      <div
        className="review-quote-modal"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header className="review-quote-header">
          <div>
            <p className="review-quote-eyebrow">Saved quote</p>
            <h2 id="review-quote-title">Review &amp; purchase</h2>
          </div>
          <button type="button" className="review-quote-close" onClick={onClose} aria-label="Close">
            ×
          </button>
        </header>

        <div className="review-quote-body">
          <QuoteSummary quote={activeQuote} premium={premium} />

          {answerRows.length > 0 ? <QuoteDetails answerRows={answerRows} /> : null}
            ...
            {!token ? (
              ...
              <>
                <p className="review-quote-wallet-balance">
                  Available balance: <strong>£{(walletBalance ?? 0).toFixed(2)}</strong>
                </p>
...
function QuoteDetails({ answerRows }: { answerRows: [string, unknown][] }) {
  return (

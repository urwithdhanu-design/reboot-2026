...
Write-Host "Granting roles/pubsub.publisher and subscriber to Cloud Run default SA: $runSa"
...
foreach ($t in $Cfg.topics) {
  ...
  $exists = $false
  gcloud pubsub topics describe $topicId --project $ProjectId 2>$null | Out-Null
  ...
  if ($CreateSubscriptions) {
    foreach ($subSvc in $t.subscribers) {
      ...
      $subExists = $false
      gcloud pubsub subscriptions describe $subId --project $ProjectId 2>$null | Out-Null
...
Write-Host "Application code must publish/subscribe using these topic names (not wired in all services yet)."
